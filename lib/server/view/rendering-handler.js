        ...
        if ('text/html' === negotiator.mediaType()) {
            resourcesController.listResourceTypes((err, types) => {
                if (err) {
                    ...
                    data.primaryNav = _.map(types, (type) => {
                        return _.extend({}, type, {active: type.text === request.params.resourceType});
                    });

                    routeRenderer.routeTo(history.createLocation(request.url), data, (error, renderedContent) => {
                        reply.view('layout/layout', {renderedContent, initialData: JSON.stringify(data)});
                    });
                }
            });
        } else {
