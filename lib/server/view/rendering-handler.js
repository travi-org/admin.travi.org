        ...
        if ('text/html' === negotiator.mediaType()) {
            ...
            resourcesController.listResourceTypes((err, types) => {
                if (err) {
                    ...
                    const
                        primaryNav = setActiveStateFor(types, request.params.resourceType),
                        data = _.extend({}, request.response.source, {primaryNav});
                    store.dispatch({
                        type: 'SET_PRIMARY_NAV',
                        nav: primaryNav
                    });

                    routeRenderer.routeTo(request.url, data, store, (error, renderedContent) => {
                        reply.view(
                            'layout/layout',
                            {
                                renderedContent,
