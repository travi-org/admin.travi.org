...
export function handler(request, reply) {
    ...
    if (response.isBoom && SERVER_ERROR === response.output.statusCode && 'text/html' === negotiator.mediaType()) {
        const
            store = configureStore(),
            renderedContent = renderToString(<Root store={store}><ErrorPage/></Root>);

        return store.dispatch(loadNav(store.getState()))
            .then(() => respond(reply, {renderedContent, store, boomDetails: {statusCode: response.output.statusCode}}))
            .catch((e) => {
                respond(reply, {renderedContent, store, boomDetails: {statusCode: SERVER_ERROR}});
